...
def run_installer():
    ...
    # Last option is excluded from available list
    config_filename = config_loader.CONFIG_OPTIONS[get_option(1, len(config_loader.CONFIG_OPTIONS) - 1) - 1]
    ...
    report_host = patterned_answer(
        "Example: %s" % ("localhost" if install_web else "webinterface.nodemon.com:8080"),
        '[a-zA-Z0-9\._\-]+',
        "localhost" if install_web else "")
    ...
    while True:
        ...
        # Now we are sure that we have only one /nodemon on the end of the path
        data_dir = os.path.join(data_dir, nodemon.PACKAGE_NAME.lower())
